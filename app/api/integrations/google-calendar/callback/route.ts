...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!tokenResponse.ok) throw new Error("Google token exchange failed"); const token = await tokenResponse.json() as { access_token: string; refresh_token?: string; expires_in?: number; scope?: string };
    const calendarsResponse = await fetch("https://www.googleapis.com/calendar/v3/users/me/calendarList", { headers: { Authorization: `Bearer ${token.access_token}` }, cache: "no-store" }); if (!calendarsResponse.ok) throw new Error("Google calendar list failed"); const calendars = await calendarsResponse.json() as { items?: { id: string; summary?: string; primary?: boolean }[] }; const calendar = calendars.items?.find((item) => item.primary) || calendars.items?.[0]; if (!calendar) throw new Error("No Google Calendar was available");
  ...
  } catch { return NextResponse.redirect(new URL("/dashboard?error=Google+Calendar+could+not+be+connected.", url)); }
}
