...
import type { Strategy } from "../../../../lib/business-diagnosis";

export const runtime = "nodejs";

export async function POST(request: Request) {
  let body: { id?: unknown; businessName?: unknown; report?: unknown }; try { body = await request.json(); } catch { return NextResponse.json({ error: "Invalid request." }, { status: 400 }); }
  ...
  try {
    const html = await bookingPrototypeHtml(businessName, body.report as Strategy);
