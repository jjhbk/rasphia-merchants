...
import { strategyShareReportHtml } from "../../../lib/strategy-share-report";
...
export async function POST(request: Request) {
  let body: { input?: unknown; email?: unknown; answers?: unknown; action?: unknown; diagnosisId?: unknown; preparedResearch?: unknown; preparedReport?: unknown; questions?: unknown; preparedBlobUrl?: unknown; preparedShareUrl?: unknown }; try { body = await request.json(); } catch { return NextResponse.json({ error: "Invalid request." }, { status: 400 }); }
  ...
  try {
    ...
    const report: Strategy = canReusePrepared && preparedReport ? preparedReport : await strategyFor(research, answers); const suppliedId = typeof body.diagnosisId === "string" ? body.diagnosisId : ""; const id = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(suppliedId) ? suppliedId : randomUUID(); let blobUrl: string | null = canReusePrepared && typeof body.preparedBlobUrl === "string" ? body.preparedBlobUrl : null; const blobShareUrl = "/api/diagnoses/report/" + id;
    if (!canReusePrepared) { try { if (process.env.BLOB_READ_WRITE_TOKEN) { const blob = await put(`diagnoses/${id}/prepared-report.json`, JSON.stringify({ id, status: "prepared", research, answers, report, preparedAt: new Date().toISOString() }, null, 2), { access: "public", contentType: "application/json", addRandomSuffix: false, allowOverwrite: true }); blobUrl = blob.url; await put(`diagnoses/${id}/strategy-report.html`, strategyShareReportHtml(research, report), { access: "public", contentType: "text/html", addRandomSuffix: false, allowOverwrite: true }); } } catch (error) { console.error("Could not archive prepared diagnosis in Vercel Blob", error); } }
    if (body.action === "prepare") return NextResponse.json({ id, research, report, blobUrl, blobShareUrl });
