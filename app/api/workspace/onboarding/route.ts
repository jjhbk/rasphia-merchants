...
export async function POST(request: Request) {
  ...
  let body: { businessName?: unknown; businessType?: unknown; timezone?: unknown; businessEmail?: unknown; channels?: unknown; firstServiceName?: unknown; firstServiceDescription?: unknown; firstServiceDuration?: unknown };
  ...
  const firstServiceDescription = typeof body.firstServiceDescription === "string" ? body.firstServiceDescription.trim().slice(0, 500) : null;
  ...
  if (!isSupportedTimezone(timezone)) return NextResponse.json({ error: "Choose a supported timezone from the list." }, { status: 422 });
  ...
  try {
    await sql.begin(async (tx) => {
      ...
      if (firstServiceName) await tx`insert into booking_services (workspace_id, name, description, duration_minutes, sort_order) select ${session.workspaceId}, ${firstServiceName}, ${firstServiceDescription}, ${firstServiceDuration}, 0 where not exists (select 1 from booking_services where workspace_id = ${session.workspaceId})`;
    });
