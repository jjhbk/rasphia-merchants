...
export async function POST(request: Request, { params }: { params: Promise<{ provider: string; workspaceId: string }> }) {
  ...
  try {
    ...
    const eventType = provider === "stripe" ? event.type || "unknown" : event.event || "unknown";
    const providerLinkId = provider === "stripe" ? event.data?.object?.id : event.payload?.payment_link?.entity?.id;
    const razorpayPaymentId = provider === "razorpay" ? event.payload?.payment_link?.entity?.payment_id || event.payload?.payment?.entity?.id || event.id || "unknown" : null;
    const eventId = provider === "stripe" ? event.id || "unknown" : `${eventType}:${subscriptionId || providerLinkId || razorpayPaymentId || "unknown"}:${razorpayPaymentId || ""}`;
    const paymentLinkStatus = paymentLinkEventStatus(provider, eventType);
    ...
    if (inserted.length && providerLinkId && paymentLinkStatus) {
      const paidLinks = await sql<{ customer_id: string; description: string; amount: number; currency: string }[]>`update payment_links set status = ${paymentLinkStatus}, provider_payment_id = coalesce(${providerPaymentId}, provider_payment_id), paid_at = case when ${paymentLinkStatus === 'paid'} then now() else paid_at end, updated_at = now() where workspace_id = ${workspaceId} and provider = ${provider} and provider_link_id = ${providerLinkId} returning customer_id, description, amount, currency`;
      ...
      if (paymentLinkStatus === "paid" && paidLinks[0]?.customer_id) {
        ...
        const detail = details[0]; const amount = `${(paidLinks[0].amount / 100).toFixed(2)} ${paidLinks[0].currency}`; const message = `Payment confirmed for ${paidLinks[0].description}. Amount received: ${amount}. Thank you—your payment has been recorded.`;
        if (detail) {
          const merchantMessage = `Payment received from ${detail.customer_name || "a customer"}. Offer: ${paidLinks[0].description}. Amount: ${amount}. The payment link is now marked paid in your Rasphia dashboard.`;
          const emailJobs = [detail.customer_email ? sendCustomerEmail({ businessName: detail.business_name, senderSlug: detail.workspace_slug, to: detail.customer_email, customerName: detail.customer_name, body: message }) : Promise.resolve(null), detail.business_email ? sendCustomerEmail({ businessName: detail.business_name, senderSlug: detail.workspace_slug, to: detail.business_email, customerName: "team", body: merchantMessage }) : Promise.resolve(null)];
