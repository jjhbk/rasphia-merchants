...
async function recordInbound(sql: postgres.Sql, message: IncomingMessage, customerName: string | null) {
  ...
  if (inserted.length) await sql`insert into customer_events (workspace_id, customer_id, event_type, source, payload) values (${workspaceId}, ${customers[0].id}, 'whatsapp_message_received', 'whatsapp', ${sql.json({ conversationId: conversations[0].id, messageId: message.id })})`;
}
