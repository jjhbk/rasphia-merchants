...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existingCustomers = customerPhone ? await sql<{ id: string }[]>`select id from customers where workspace_id = ${session.workspaceId} and (lower(email) = lower(${customerEmail}) or phone = ${customerPhone}) order by case when lower(email) = lower(${customerEmail}) then 0 else 1 end limit 1` : await sql<{ id: string }[]>`select id from customers where workspace_id = ${session.workspaceId} and lower(email) = lower(${customerEmail}) limit 1`;
    const customers = existingCustomers.length ? await sql<{ id: string }[]>`update customers set email = ${customerEmail}, phone = coalesce(${customerPhone || null}, phone), first_name = ${customerName}, updated_at = now() where id = ${existingCustomers[0].id} returning id` : await sql<{ id: string }[]>`insert into customers (workspace_id, email, phone, first_name, source) values (${session.workspaceId}, ${customerEmail}, ${customerPhone || null}, ${customerName}, 'dashboard_payment') returning id`;
  ...
  } catch (error) { console.error("Payment checkout failed", error); return NextResponse.json({ error: error instanceof Error ? error.message : "We couldn’t create that payment link." }, { status: 422 }); }
