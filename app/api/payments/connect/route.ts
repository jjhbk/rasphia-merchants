...
export async function POST(request: Request) {
  ...
  const provider = body.provider === "stripe" || body.provider === "razorpay" ? body.provider : null; const webhookSecret = typeof body.webhookSecret === "string" ? body.webhookSecret.trim() : ""; const requestedCurrency = typeof body.currency === "string" ? body.currency.toUpperCase() : "USD"; const currency = isSupportedCurrency(requestedCurrency) ? requestedCurrency : null;
  ...
  if (!provider || !currency || !webhookSecret || (provider === "stripe" && !stripeSecretKey) || (provider === "razorpay" && (!razorpayKeyId || !razorpayKeySecret))) return NextResponse.json({ error: "Add the required provider credentials, supported currency, and webhook secret." }, { status: 422 });
