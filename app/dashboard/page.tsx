...
function DashboardNav({ name, slug }: { name: string; slug: string }) { return <aside className="dashboard-nav"><Link className="wordmark" href="/">rasph<em>ia</em></Link><p>{name}</p><nav><Link href="/dashboard">Overview</Link><Link href={`/book/${slug}`}>Booking page</Link><span>Conversations</span><span>Customers</span><span>Workflows</span><span>Payments</span><span>Knowledge base</span><span>Settings</span></nav><form action="/api/auth/signout" method="post"><button type="submit">Sign out</button></form></aside>; }
