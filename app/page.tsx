...
const pillars: [string, string, string, string[]][] = [
  ["01 · Growth", "Get found. Get chosen.", "Be visible where customers look, then use the acquisition channel most likely to pay back for you.", ["Search, maps, social & AI visibility", "Ads, partners, or direct outreach", "Campaigns launched and measured"]],
  ["02 · Retention", "Keep them coming back.", "Spot quiet customers, fill spare capacity, and make follow-up a dependable part of your business.", ["Lapsed-customer win-backs", "Smart offers for quiet times", "Bookings, reminders & replies"]],
  ["03 · Revenue", "Make every booking count.", "Collect money without chasing and turn a good first visit into repeat, predictable revenue.", ["Deposits & payment links", "No-show protection", "Packages & memberships"]],
];
...
export default function Home() {
  return <main>
    ...
    <header id="top" className="hero"><div className="wrap hero-grid">
      <div className="hero-copy"><p className="eyebrow">Growth · Retention · Revenue</p><h1>You don’t need <span>more marketing</span>.<br />You need the <em>right move</em>.</h1><p className="lede">Rasphia finds where your business is losing momentum, identifies the few actions most likely to pay back, and runs them with you. No generic plan. No pile of tools. Just the next moves that matter.</p><div className="actions"><Link className="button button-gold" href="/diagnosis">Get your free diagnosis</Link><a className="button button-outline" href="#how">See how it works</a></div><p className="note">Free diagnosis · No commitment · For local service businesses</p></div>
      <DiagnosisPreview />
    </div></header>

    <div className="strip"><div className="wrap strip-inner"><span>Diagnoses <b>your business</b>, not a category</span><span>Shows what <b>not to spend on</b></span><span>Turns the right move into <b>action</b></span></div></div>
    ...
    <section className="dark"><div className="wrap"><p className="section-label">Why it works</p><h2>The most useful recommendation can be the money you don’t spend.</h2><div className="reasons"><article><h3>Your data, not a generic playbook</h3><p>Pricing, bookings, capacity, customers, reviews, and what is already working provide the context for every recommendation.</p></article><article><h3>Advice that becomes action</h3><p>A report is only useful when it gets done. Rasphia helps launch the message, campaign, booking flow, or payment setup.</p></article><article><h3>One system, not seven subscriptions</h3><p>Keep the customer journey connected instead of handing it between a booking app, payment tool, marketing platform, and agency.</p></article></div></div></section>

    <section className="steps-section"><div className="wrap"><p className="section-label">Getting started</p><h2>Fifteen minutes to see where growth is getting stuck.</h2><div className="steps"><article><b>1</b><h3>Share the picture</h3><p>Tell us about your business, your customers, and where you feel stuck.</p></article><article><b>2</b><h3>See your diagnosis</h3><p>We identify the leaks, rank the next moves, and show what is not worth doing yet.</p></article><article><b>3</b><h3>Choose what to run</h3><p>Pick the moves that feel right. Rasphia helps put them into motion.</p></article></div></div></section>
